...
export function BookingFlow({ initialService, initialProvider, services, providers, categories, liveSquare }: Props) {
  ...
  return <div className="container-site py-10 sm:py-16">
    ...
    <div className="grid gap-7 lg:grid-cols-[1fr_330px]">
      <div className="paper-card min-h-[560px] p-5 sm:p-8">
        ...
        {step === 4 && <form onSubmit={submitDetails}><button type="button" onClick={() => setStep(3)} className="mb-6 flex items-center gap-2 text-xs font-bold text-[#676767]"><ArrowLeft size={14} /> Change time</button><p className="text-xs font-bold uppercase tracking-[.14em] text-[#893d3e]">Your details</p><h2 className="font-display mt-2 text-3xl">Almost there.</h2><div className="mt-7 grid gap-4 sm:grid-cols-2"><Field label="First name" name="firstName" autoComplete="given-name" /><Field label="Last name" name="lastName" autoComplete="family-name" /><Field label="Email address" name="email" type="email" autoComplete="email" /><Field label="Mobile number" name="phone" type="tel" autoComplete="tel" /></div><label className="mt-4 block text-xs font-bold text-[#33373d]">Anything we should know? <span className="font-normal text-[#777777]">Optional</span><textarea name="note" maxLength={500} rows={4} className="mt-2 w-full resize-none rounded-xl border border-[#d8cfc5] bg-white p-4 font-normal outline-none focus:border-[#893d3e]" /></label>{submitError && <div role="alert" className="mt-5 rounded-2xl bg-[#f8e8e5] p-4 text-sm leading-6 text-[#713335]">{submitError}</div>}<button disabled={submitting} className="button-primary mt-6 w-full disabled:cursor-wait disabled:opacity-60" type="submit">{submitting ? <><LoaderCircle size={15} className="animate-spin" /> Confirming…</> : <>Confirm appointment <ArrowRight size={15} /></>}</button><p className="mt-4 flex items-center justify-center gap-1.5 text-[.68rem] text-[#777777]"><LockKeyhole size={12} /> Your appointment is created securely in Square. No payment is collected today.</p></form>}
        {step === 5 && booking && <div className="grid min-h-[500px] place-items-center text-center"><div><span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-[#f6f5e9] text-[#893d3e]"><CheckCircle2 size={30} /></span><p className="mt-6 text-xs font-bold uppercase tracking-[.15em] text-[#893d3e]">Appointment confirmed</p><h2 className="font-display mt-3 text-4xl">We’ll see you soon.</h2><p className="mx-auto mt-4 max-w-md text-sm leading-7 text-[#676767]">Your {service?.name} appointment with {provider?.name} is booked for {formatAppointment(booking.startsAt)}.</p><p className="mt-4 text-xs text-[#777777]">Confirmation: <span className="font-mono font-bold text-[#33373d]">{booking.id}</span></p><p className="mx-auto mt-5 max-w-md text-xs leading-6 text-[#777777]">Need to make a change? Call the spa at <a className="font-bold text-[#893d3e]" href="tel:+17166523080">(716) 652-3080</a>.</p><button onClick={() => { setStep(1); setComplete(false); setServiceSlug(""); setSelectedTime(""); setSelectedProviderId(""); resetSubmission(); }} className="button-secondary mt-7">Book another visit</button></div></div>}
      </div>
